...
import com.iwolt.games.pengpeng.control.CButton;
import com.iwolt.games.pengpeng.control.JoyStick;
import com.iwolt.games.pengpeng.enemies.Enemy;
import com.iwolt.games.pengpeng.enemies.Enemy1;
import com.iwolt.games.pengpeng.enemies.Enemy2;
import com.iwolt.games.pengpeng.panel.BulletCounter;
import com.iwolt.games.pengpeng.panel.CoinCounter;
import com.iwolt.games.pengpeng.panel.HealthBar;
import com.iwolt.games.pengpeng.panel.ScoreCounter;
...
public class Level extends World {
	...
	protected void create() {
		...
		stage.addActor(pengpeng);
		
		
		//camera follow hero
		camController.followObject(pengpeng);
		...
		//on screen button
		jumpBtn = new CButton(new Image(SPengPeng.atlas.findRegion("jump")), new Image(SPengPeng.atlas.findRegion("jump_down")), minHeight);
		bulletBtn = new CButton(new Image(SPengPeng.atlas.findRegion("bullet_btn")), new Image(SPengPeng.atlas.findRegion("bullet_btn_down")), minHeight);
	...
	private void createEntities(MapObjects objects) {
		...
		for (MapObject obj : objects) {
			...
			if(name.equals("pengpeng")) {
				//create hero
				pengpeng = new Pengpeng(this);
				pengpeng.setPosition(rect.x + rect.width/2, rect.y + pengpeng.getHeight()/2);
				addEntity(pengpeng);
				pengpeng.addListener(heroListener);
			}
	...
	protected void updatePanel() {
		healthBar.setValue(pengpeng.getHealthRatio());
	...
	private void enemyHitEnemy(Enemy enemyA , Enemy enemyB) {
		...
		if(enemyA instanceof Enemy2) {
			if(((Enemy2)enemyA).isSliding()) {
				...
				SPengPeng.media.playSound("hit2");
				return;
			}
		} 
		//turtle enemy slide other enemy
	...
	private void heroHitObject(Entity obj) {
		...
		if(obj instanceof Coin) {
			...
			SPengPeng.media.playSound("coin");
		...
		if(obj instanceof BulletStock) {
			...
			SPengPeng.media.playSound("bullet_pack");
		...
		if(obj instanceof Enemy) {
			...
			if(pengpeng.v.y < -10 && pengpeng.getBottom() > obj.getY()) {
				Enemy enemy = (Enemy)obj;
				pengpeng.stepEnemy(enemy);
				enemy.attackedByHero(pengpeng,1);
				...
				//enemy attack hero
				if(!pengpeng.isImmune()) {
					
					pengpeng.attackedBy(((Enemy)obj));
					((Enemy)obj).attackHero(pengpeng);
					SPengPeng.media.playSound("hit2");
	...
	private void heroReachFlag() {
		...
		pengpeng.gameCompleted();
		
		score += 500;
		totalScore += 500;
		updatePanel();
		toastLabel("500", flag.getX() ,pengpeng.getTop());
		
		flag.down();
		
		SPengPeng.media.playSound("flag");
	...
	private void resumeLevel() {
		...
		SPengPeng.media.unMuteAllMusic();
		call(SPengPeng.LEVEL_RESUMED);
	}
	...
	private void pauseLevel(boolean withdialog) {
		...
		SPengPeng.media.muteAllMusic();
		//track the event
		call(SPengPeng.LEVEL_PAUSED);
	}
	...
	public void heroFireBullet(boolean toRight) {
		...
		bullet.setPosition(pengpeng.getX(), pengpeng.getY());
	...
	protected void levelCompleted() {
		...
		SPengPeng.data.setLevelProgress(id);
		
		//update the score
		SPengPeng.data.updateScore(id, score);
		...
		if (SPengPeng.data.getStar(id) < stars) {
			SPengPeng.data.saveStar(id, stars);
		}

		LevelCompletedDialog dialog = new LevelCompletedDialog(score, stars);
		addOverlayChild(dialog);
		centerActorXY(dialog);
		...
		SPengPeng.media.playSound("level_completed");
		SPengPeng.media.stopMusic("level");
		call(SPengPeng.LEVEL_COMPLETED);
	}
	...
	protected void update(float delta) {
		...
		if(state == GAME_PLAY) {
			...
			pengpeng.onKey(lKeyLeft, lKeyRight,lKeyJump,lKeyFire);
			//hero fall 
			if(pengpeng.getBottom() < 0) {
				pengpeng.fall();
			}
		
		}
		else if(state == GAME_COMPLETED) {
			pengpeng.onKey(false, false,false,false);

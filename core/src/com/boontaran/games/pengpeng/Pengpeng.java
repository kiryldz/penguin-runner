...
public class Pengpeng extends Entity {
	...
	public Pengpeng(Level level) {
		this.level = level;
		
		//construct the clip, and clip listener
		clip = new Clip(SPengPeng.atlas.findRegion("pengpeng") , 120,120);
		setSize(50	, 95);
		setClip(clip);
		clip.setFPS(12);
	...
	private void jump() {
		...
		//sound
		SPengPeng.media.playSound("jump");
	}
	private void fireBullet() {
		...
		SPengPeng.media.playSound("bullet");
	}
	...
	public void update(float delta) {
		...
		if(starTimer > 0) {
			...
			if(starTimer <= 0) {
				...
				SPengPeng.media.stopMusic("star");
			}
		}
